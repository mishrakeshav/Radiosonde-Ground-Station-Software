...
class Dashboard(object):
    def setupUi(self, MainWindow):
        ...
        self.data_frame = pd.read_csv(os.path.join('/home/keshav/Desktop/application/Radiosonde-Ground-Station-Software/src/export/20201008_024420/output.csv'))
        
        self.timer.start()
        self.update_gauge(*[0, 0, 0, 0, 0, 0])

        self.update_tephigraph()
    ...
    def update_tephigraph(self):
        # self.data_frame.rename(columns = {'External Temperature':'Temperature','Wind Speed':'WindSpeed','Wind Direction':'WindDirection'}, inplace = True)
        self.data_frame['Td'] = self.data_frame.Temperature - ((100 - self.data_frame.Humidity)/5)
        dewpoint = list(zip(data_frame['External Temperature'],data_frame['Td']))
        drybulb = list(zip(data_frame['Pressure'], data_frame['External Temperature']))
        drybulb = list(zip(data_frame['Pressure'], data_frame['External Temperature']))
        barbs = list(zip(data_frame['Wind Speed'],data_frame['Wind Direction'], data_frame['Pressure']))
